    ...
    handleKeyUp= (e)=>{
        ...
        if (keyCode === 38 || keyCode === 40) {
           if (keyCode === 38){
            ...
            e.target.value=this.state.arr[this.state.index];
        }
    }
    handleKeyDown= (e)=>{
        if (e.keyCode ===13){
            ...
            <div className='container' style={style}>
                ...
                <ul className='list-group'>
                    {this.state.arr.map((item,key)=>{
                      return  <li onClick={this.handleClick(key)} className={key===this.state.index ? 'list-group-item active' :"list-group-item"} key={key}>{item}</li>
                    })}
                </ul>
            </div>
        )
